...
RAIBITSERVER_OAUTH_RELAY_SECRET=<independent-32-random-bytes-as-64-lowercase-hex>
...
7. **GitHub App/OAuth 연결**
   - OAuth 트랜잭션 저장소는 `000011_oauth_transactions` 적용 후 사용할 수 있습니다. 상태는 SHA-256, 소스는 서버 전용 `sourceSecret`(32자 이상)의 HMAC-SHA-256으로 저장하고 S256 challenge·정확한 redirect URI·최대 10분 TTL을 묶습니다. redirect에는 query/fragment/사용자 정보가 없어야 하며 HTTPS 또는 로컬 loopback HTTP만 허용합니다. 메모리/Prisma의 `createOAuthTransaction`, `consumeOAuthTransaction`, `deleteExpiredOAuthTransactions`는 일회 소비와 최대 256행 만료 정리를 제공합니다. 마이그레이션을 먼저 적용하고 readers/writers를 배포하며, 앱 롤백 시 테이블은 유지하고 수정은 forward-fix로 진행합니다. 공개 시작·콜백/BFF 바인딩과 남용 방지는 아래 절차를 사용합니다. 실제 PostgreSQL 검증은 `RAIBITSERVER_TEST_DATABASE_URL`을 지정해 `node --test tests/oauth-transaction.test.js tests/postgres-oauth-transaction.test.js`로 실행합니다. 기본 로컬 테스트에서 URL이 없으면 PostgreSQL 시나리오는 `NOT_RUN`으로 건너뜁니다. 필수 검증에는 `RAIBITSERVER_REQUIRE_POSTGRES_TESTS=1`도 설정해야 하며, 이때 URL 누락은 실패입니다. CI의 전용 PostgreSQL job은 이 필수 모드로 순차 실행합니다.
   - 공개 OAuth 시작·콜백은 위 저장소를 사용합니다. 직접 API 클라이언트는 43~128자 verifier의 SHA-256 base64url challenge를 `GET /auth/github/login?codeChallenge=...`에 보내고, 서버가 반환한 state와 원래 verifier를 `GET /auth/github/callback?code=...&state=...&codeVerifier=...`에 보내야 합니다. caller state, challenge 누락, 재사용·만료·다른 소스·다른 redirect는 거부되며 GitHub 호출 전에 원자적으로 소비됩니다. provider 실패나 계정 정책 거부 이후에도 다시 사용할 수 없습니다. 거절 콜백은 code 대신 `error=access_denied`를 같은 state/verifier와 함께 보내며 트랜잭션을 소비한 뒤 400을 반환합니다. code와 error를 함께 보내면 소비 없이 400, 이미 소비한 콜백은 409입니다.
   - OAuth 시작·콜백은 각각 source/action 30회, 전체 인증 flow/source 60회, global 5000회/분의 기존 durable bucket을 한 번 차감합니다. 공급자 검증 이메일이 알려진 콜백은 이메일/action 10회/분도 차감합니다. 기존 한도 설정과 1~3600초의 `Retry-After`를 유지합니다. 직접 API 요청은 실제 소켓 피어를 사용하고, Dashboard가 서명한 요청은 브라우저별 식별값을 사용하므로 같은 Dashboard를 쓰는 다른 브라우저가 30회 한도를 공유하지 않습니다. `X-Forwarded-For`나 일반 proxy 신뢰 플래그는 OAuth 출처를 바꾸지 않습니다. 브라우저 쿠키 삭제로 새 식별값을 얻을 수 있으므로 이는 브라우저 간 격리이며, 공격자가 여러 식별값을 만드는 것까지 막지는 않습니다. global/email 한도와 배포 환경의 외부 요청 제한도 유지해야 합니다. 기존 signup/login/email의 proxy 정책은 바꾸지 않습니다.
   - OAuth 결과를 분류한 뒤 최대 256개의 만료 트랜잭션을 정리하고 고정 결과·오류 코드·정리 상태만 감사 저장합니다. 성공 응답은 정리와 감사 저장까지 성공해야 전달됩니다. 감사 장애는 안전한 503, 정리 장애는 기존 인증 실패를 유지하거나 성공 예정 응답을 503으로 차단합니다. 사용 가능한 서버 키·저장소가 없으면 인증을 차단하며 감사 저장소 자체가 없을 때는 이벤트 저장을 보장하지 않습니다. state/code/verifier/token/email/원문 소스와 credential은 감사·로그에 넣지 않습니다. 잘못된 브라우저 쿠키·미지원 거절은 BFF에서 차단되므로 API 감사 범위 밖입니다. `node --test tests/oauth-abuse.test.js tests/oauth-redaction.test.js tests/security-hardening.test.js`가 내구성 한도·정리·오류 비식별화를 검증합니다.
   - `RAIBITSERVER_GITHUB_REDIRECT_URI`는 OAuth callback URL 한 개로 고정합니다. Dashboard는 `https://console.<BASE_DOMAIN>/api/control/auth/github/callback`을 사용하며 요청에 다른 redirect를 지정하면 거부합니다. 소스 HMAC 키는 명시된 `RAIBITSERVER_AUTH_RATE_LIMIT_KEY_SECRET`, 없으면 실제 JWT secret을 사용합니다. 둘 다 32~512자여야 하고 명시된 잘못된 키는 fallback하지 않습니다.
   - Dashboard와 API에 별도의 `RAIBITSERVER_OAUTH_RELAY_SECRET`을 동일하게 설정합니다. 32바이트 난수를 소문자 hex 64자로 생성하고 JWT 서명 키와는 분리합니다. Helm은 `runtimeSecrets.existingSecret`의 이 키만 Dashboard에 주입하며, API는 기존 `envFrom`으로 읽습니다. OAuth를 쓰지 않는 설치에서는 키가 없어도 배포할 수 있지만 Dashboard의 GitHub 로그인은 키가 없거나 잘못되면 차단됩니다. API 검증 코드를 먼저 배포한 뒤 Dashboard를 배포하세요. 키 변경이나 쿠키 전환 때 진행 중이던 로그인은 처음부터 다시 시작해야 합니다.
   - Dashboard는 서명된 `__Host-raibitserver_github_oauth_browser` 세션 쿠키를 유지하고, 해당 식별값·요청 종류·쿼리·발급 시각을 묶은 `X-Raibitserver-OAuth-Relay` 헤더를 API에 보냅니다. API는 30초 이내 서명만 수용하며 잘못된 헤더를 소켓 출처로 대체하지 않습니다. 같은 식별값이 거래 생성과 소비에 쓰이므로 Dashboard 복제본이 바뀌어도 유지됩니다. 클라이언트가 보낸 relay 헤더와 중복된 OAuth 쿠키는 Dashboard가 거부합니다. 서명 헤더와 쿠키 값은 로그에 기록하지 마세요.
   - state/verifier 쿠키도 `__Host-raibitserver_github_oauth_state`, `__Host-raibitserver_github_oauth_verifier`를 사용합니다. 세 OAuth 쿠키 모두 Secure·HttpOnly·SameSite=Lax·`Path=/`이며 Domain을 설정하지 않습니다. 브라우저가 하위 도메인의 상위 Domain 쿠키 주입을 거부하도록 `__Host-` 규칙을 따르며 이전 이름의 쿠키는 인증에 사용하지 않습니다. 모든 callback 결과에서 state/verifier는 지우고 브라우저 식별 쿠키는 유지합니다. 로컬 브라우저 검증에도 HTTPS가 필요합니다.
   - 새 OAuth 검증: `node --test tests/github-oauth.test.js tests/github-oauth-avatar.test.js tests/auth-env-github.test.js`. 브라우저 출처 분리 회귀는 `node --test tests/oauth-proxy-isolation.test.js apps/dashboard/lib/github-oauth-relay.test.mjs`로 실행합니다. 실제 HTTPS 쿠키 주입 검사는 설치된 Chromium과 OpenSSL로 `node --import ./tests/fixtures/resource-runtime.mjs --test apps/dashboard/tests/e2e/specs/pr17-security-oauth-browser.spec.mjs`를 실행합니다(`RAIBIT_OAUTH_CHROMIUM`, `RAIBIT_PR17_OPENSSL`로 실행 파일 지정 가능). 이 검사는 실제 Next 요청 처리기와 Nest HTTP를 사용하지만 화면은 테스트용이며, 전체 Next 빌드·화면 검증을 대신하지 않습니다. 전용 `apps/dashboard/playwright.oauth.config.ts`는 외부 evidence 디렉터리와 로컬 HTTPS fixture manifest를 사용합니다. GitHub provider는 로컬 fixture로 대체하고, OAuth 구간 tracing은 끄며 보관 결과에는 비밀 값을 넣지 않습니다.
